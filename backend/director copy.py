...
class VideoDirector:
    ...
    def generate_plan(self, user_prompt: str):
        ...
        with open("final_plan.json", "r") as f:
            final_plan = json.load(f)

        track_id = final_plan["selected_track_filename"].split(".")[0][-1]
        print(track_id)
        selected_file_id = paths[int(track_id)-1]

        print(selected_file_id)
        os.makedirs("trimmed_audio", exist_ok=True)
        output_path = os.path.join("trimmed_audio", final_plan["selected_track_filename"])
        VideoDirector.trim_audio(selected_file_id, output_path, final_plan["start_time_seconds"], final_plan["end_time_seconds"])
        VideoDirector.send_audio_to_phone(output_path)
